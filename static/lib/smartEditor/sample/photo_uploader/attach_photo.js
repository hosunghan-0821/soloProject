    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/saveImage'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		fdata.method="POST"
		fdata.append("filedata",tempFile)
		console.log(123);
	...
	function customGeneralUpload(){
		let imgFile = jQuery3_4_1("#uploadInputBox").files[0];
		let fdata = new FormData();
		fdata.enctype="multipart/form-data";
		fdata.method="POST"
		...
		jQuery3_4_1.ajax({
			...
			success: function (data){
				...
				if(data.result=="200"){
					...
					aResult.push(obj)
					setPhotoToEditor(obj)
					goReadyMode()
					window.close()
				}
				else{
					alert('사진 업로드를 실패했습니다1.')

				}
			}
			,error: function (xhr,testStatus, errorThrown){
				alert('사진 업로드를 실패하였습니다2.')
			}
		})
